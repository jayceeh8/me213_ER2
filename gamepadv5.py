...
ble_client = None
ble_loop = None          # <-- stores the BLE thread's event loop
...
async def ble_connect():
    ...
    # Capture the event loop from inside the async context
    ...
    # Keep the loop alive so send() can schedule work into it
...
def send(cmd):
    ...
    future = asyncio.run_coroutine_threadsafe(
        ...
        ble_loop   # <-- use the BLE thread's loop, not the main thread's
    )

    try:
        future.result(timeout=2)   # block until sent or timeout
        print(f"[Sent]: {cmd}")
    ...
    while True:
        ...
        if estop:
            ...
            if arm_up and not arm_down:
                ...
                print("ARM DOWN PYTHON")
            else:
                arm_command = "ARM_STOP"

        # Only send if the command changed
